...
public class ShooterGameCamera : MonoBehaviour {
	...
	void LateUpdate () {
		...
		angleH += Mathf.Clamp(Input.GetAxis("Mouse X") + Input.GetAxis("Horizontal"), -1, 1) * horizontalAimingSpeed * Time.deltaTime;
		angleV += Mathf.Clamp(Input.GetAxis("Mouse Y") + Input.GetAxis("Vertical"), -1, 1) * verticalAimingSpeed * Time.deltaTime;
		...
		// RandomMove camera towards closeOffset if ray back towards camera position intersects something 
		RaycastHit hit;
